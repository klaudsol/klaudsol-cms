...
class Entity {
    ...
    static async find({entity_type_slug, id}) {
      ...
      const sql = `SELECT 
                  ...
                  LEFT JOIN images ON images.entity_id = entities.id AND images.value_id = values.id AND images.attribute_id = attributes.id
                  WHERE 
      ...
      return data.records.map(([
          ...
          {stringValue: images_name},
          {stringValue: images_link}
        ]) => ({
          id, 
          entity_type_id,
          entity_type_name, 
  ...
  static async delete({id}) {
    ...
    const deleteImagesSQL = 'DELETE from images where entity_id = :id'
    const getValuesSQL = 'SELECT * FROM \`values\` where entity_id = :id'

    let executeStatementParam = [
      { name: 'id', value: { stringValue: id } }
    ]
    await db.executeStatement(deleteImagesSQL, executeStatementParam);
