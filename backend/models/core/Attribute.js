...
export default class Attribute {
  ...
  static async create({ entity_type_id, name, type, order }) {
    ...
    const insertSQL =
      "INSERT into attributes (`entity_type_id`, `name`, `type`, `order`) VALUES (:entity_type_id, :name, :type, :order)";
  ...
  static async deleteWhere({ type_slug, name }) {
    ...
    const db = new DB();
    const sql = `SELECT \`values\`.value_string                    
                 FROM entities
                 LEFT JOIN entity_types ON entities.entity_type_id = entity_types.id
                 LEFT JOIN attributes ON attributes.entity_type_id = entity_types.id
                 LEFT JOIN \`values\` ON values.entity_id = entities.id AND values.attribute_id = attributes.id
                 WHERE entity_types.slug = :type_slug AND attributes.type = "image" AND attributes.name = :name;`
                   
    const imagesData = await db.executeStatement(sql, [
      { name: "type_slug", value: { stringValue: type_slug } },
      { name: "name", value: { stringValue: name } },
    ]);
    const imagesNames =  imagesData.records.map(([{stringValue:key}])=>(key))

    const entityType = await EntityType.findBySlug(type_slug, { db });
    const entityTypeId = entityType.entity_type_id;

    const deleteSQL =
      "DELETE FROM attributes WHERE `entity_type_id` = :entityTypeId AND `name` = :name";
  ...
  static async updateWhere({ type_slug, name: oldName, attribute }) {
    ...
    const updateSQL =
      "UPDATE attributes SET `name` = :name, `type` = :type, `order` = :order WHERE `entity_type_id` = :entity_type_id AND `name` = :old_name";
