...
const AppContentManagerTable = ({columns, entries, entity_type_slug}) => {
    ...
    const checkEntryIfObject = (entry, accessor) => {
        if(typeof entry[accessor] === 'object') return entry[accessor].originalName; 
            ...
            {entries.map((entry, i) => (
                <tr key={i}>
                    {columns.map((col, index) => (
                        <Link href={`/admin/content-manager/${entity_type_slug}/${entry.id}`} passHref key={index}><td key={index}>{checkEntryIfObject(entry, col.accessor)}</td></Link>
                    ))}
                </tr>
            ))}
            </tbody>
