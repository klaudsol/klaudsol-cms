...
import { DEFAULT_SKELETON_ROW_COUNT, writeContentTypes } from "lib/Constants";
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!rootState.entityTypes || rootState.entityTypes.length === 0)
        ...
        <div className="d-flex flex-column mx-0 px-0">
          ...
          {!state.isSkeletonVisible &&
            rootState.entityTypes.map((type, i) => (
              <Link
                href={`/admin/content-type-builder/${type.entity_type_slug}`}
                passHref
                key={i}
              >
                <button
                  data-current-type-slug={currentTypeSlug}
                  key={i}
                  className={
          ...
          {capabilities.includes(writeContentTypes) && <button
            ...
            className="content_create_button"
          >
            {" "}
            <FaPlus className="content_create_icon" /> Create new collection
            type{" "}
          </button>}
        </div>
      </div>
